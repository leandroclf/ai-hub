...
| `browser-smoke.json` | Chromium real com OIDC PKCE, senha+OTP, editor de contrato REST declarativo, CRUD/readback, navegação, SLA, redelivery administrativo com recibo/auditoria durável, destinos versionados, preparação financeira com bloqueio visual de autoaprovação, logout, leitor global cross-tenant com finalidade, `tenant_reader` sem mutações de destinos/protocolos/financeiro, HTTP 401 pós-logout e 390 px: PASS | Não substitui matriz completa de autorização nem entrega externa ponta a ponta |
