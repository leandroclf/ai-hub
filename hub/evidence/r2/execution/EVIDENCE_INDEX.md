...
| `snapshot-admission-postgres.log` | R2-CAT-06-S01; PostgreSQL/Go race congelou config snapshot v1 no protocolo e na intenção após alteração local para v2 | Parcial; mutação efetiva de Atlas durante execução e cache/revogação ainda não foram exercitados |
...
| `admin-service-publication-smoke.json` | Ensaio Chromium curto: 4 verificações `PASS`; qualificação vigente, publicação v1, rejeição de mutação com HTTP 409 e publicação de v2 com schema incompatível preservando v1/hash | Protocolo v1 real durante o upgrade ainda não foi exercitado; fixture local |
