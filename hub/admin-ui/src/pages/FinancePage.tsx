...
export default function FinancePage({tenant,principal}:{tenant:string;principal:Principal}){
 ...
 return <section>
  ...
  {canWrite&&<fieldset disabled={busy}>
   ...
   {view==='disputes'&&<><label>ID do fato<input inputMode="numeric" value={factID} onChange={event=>setFactID(event.target.value)}/></label><label>Valor contestado<input value={amount} onChange={event=>setAmount(event.target.value)}/></label><label>Evidência<input value={evidence} onChange={event=>setEvidence(event.target.value)}/></label><label>Razão<input minLength={8} value={reason} onChange={event=>setReason(event.target.value)}/></label><button disabled={!factID||!amount||!evidence||reason.trim().length<8} onClick={()=>void action('disputes',{fact_id:factID,amount,evidence_id:evidence,reason})}>Abrir divergência</button></>}
