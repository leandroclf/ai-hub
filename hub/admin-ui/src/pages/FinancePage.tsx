...
export default function FinancePage({tenant,principal}:{tenant:string;principal:Principal}){
 ...
 return <section>
  ...
  {result&&<><DefinitionList value={result}/>{view==='adjustments'&&Array.isArray(result.items)&&<ul aria-label="Ajustes persistidos">{(result.items as RecordData[]).map(item=><li key={String(item.id)}>{String(item.id)} · {String(item.state)} · {String(item.reason)} {canApprove&&String(item.state)==='PREPARED'&&<button onClick={()=>void action(`adjustments/${encodeURIComponent(String(item.id))}/approve`,{})}>Aprovar</button>}</li>)}</ul>}{!!result.next_cursor&&<button onClick={()=>navigate(`/finance?${new URLSearchParams({view,cursor:String(result.next_cursor)})}`)}>Próxima página</button>}</>}
