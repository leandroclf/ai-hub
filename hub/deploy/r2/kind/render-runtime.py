...
root=pathlib.Path(__file__).resolve().parents[1];namespace='ai-hub-local-kind';project=os.environ.get('R2_COMPOSE_PROJECT','ai_hub_r3qual');mode=os.environ.get('R2_KIND_DEPENDENCIES','compose');compose=['docker','compose','-p',project,'-f',str(root/'compose.yaml')];docs=[{'apiVersion':'v1','kind':'Namespace','metadata':{'name':namespace}}]
if mode == 'compose':
 ...
 raise RuntimeError(f'R2_KIND_DEPENDENCIES inválido: {mode}')
data={'ENVIRONMENT':'local','CELL_ID':'r2-cell-a','OIDC_ISSUER':os.environ.get('R2_KIND_OIDC_ISSUER','http://localhost:18085/realms/ai-hub-r2'),'OIDC_JWKS_URL':'http://identity:8080/realms/ai-hub-r2/protocol/openid-connect/certs','OIDC_AUDIENCE':'ai-hub','OIDC_TOKEN_URL':'http://identity:8080/realms/ai-hub-r2/protocol/openid-connect/token','ATLAS_URL':'http://atlas:8081','ORBITA_URL':'http://orbita:8080','COMETA_URL':'http://cometa:8082','LIBRA_URL':'http://libra:8084','QUEUE_ENDPOINT':'http://localstack:4566','QUEUE_REGION':'us-east-1','QUEUE_NAMESPACE':'r2-kind-cell-a','AWS_ACCESS_KEY_ID':'local','AWS_SECRET_ACCESS_KEY':'local','AWS_REGION':'us-east-1','AWS_ENDPOINT_URL':'http://localstack:4566','REDIS_ADDR':'','OTEL_EXPORTER_OTLP_ENDPOINT':'http://alloy:4318','LOG_LEVEL':'info'}
