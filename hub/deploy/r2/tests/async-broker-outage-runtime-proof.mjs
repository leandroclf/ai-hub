...
// Qualifica a diferença entre aceite durável e publicação: ASYNC deve retornar
// 202 com a intenção preservada quando o broker está fora e concluir depois
// que a dependência é restaurada.
...
const evidencePath=resolve('hub/evidence/r2/execution/async-broker-outage-runtime-latest.json');
...
async function authenticatedToken(){
 ...
 try{
  ...
  for(let attempt=0;attempt<8&&!authenticated;attempt++){
   ...
   try{await page.waitForURL(url=>new URL(url).origin===new URL(adminOrigin).origin&&new URL(url).pathname==='/services',{timeout:3500});authenticated=true}
   catch{authenticated=await page.getByRole('heading',{name:'Serviços',exact:true}).count()>0;if(!authenticated)await page.waitForTimeout(500)}
  }
...
function fixtureIP(service){return execFileSync('docker',['inspect',`${project}-${service}-1`,'--format','{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}'],{encoding:'utf8'}).trim()}
function compose(args){return execFileSync('docker',['compose','--project-name',project,'--file',composeFile,...args],{encoding:'utf8',stdio:['ignore','pipe','pipe'],env:{...process.env,R2_PROVIDER_CIDR:`${fixtureIP('provider-sim')}/32`,R2_SINK_CIDR:`${fixtureIP('webhook-sink')}/32`,R2_IDENTITY_CIDR:`${fixtureIP('identity')}/32`}})}
...
async function wait(url,timeoutMs=45000){const deadline=Date.now()+timeoutMs;while(Date.now()<deadline){try{if((await fetch(url)).status===200)return}catch{}await new Promise(resolve=>setTimeout(resolve,500))}throw new Error(`dependência não ficou pronta: ${url}`)}
async function request(token,path,options={}){const response=await fetch(`${orbitaOrigin}${path}`,{...options,headers:{'content-type':'application/json',authorization:`Bearer ${token}`,'X-Tenant-Id':'acme',...(options.headers||{})}});return {status:response.status,body:await response.json().catch(()=>({}))}}

const prefix=process.env.R2_ASYNC_BROKER_OUTAGE_PREFIX||`r4-async-broker-outage-${Date.now()}`;
const key=`${prefix}-async-001`;
const {browser,token}=await authenticatedToken();
let result={status:'FAIL',profile:'r2-exe-01-s02-async-broker-outage',compose_project:project,key};
try{
 const before=await fetch(`${providerSim}/__qualification/effects`).then(response=>response.json());
 compose(['stop','localstack']);
 const admission=await request(token,'/v1/protocols',{method:'POST',headers:{'Idempotency-Key':key},body:JSON.stringify({mode:'ASYNC',provider_account_id:'prov-poll-1',service_code:'protocolo-assincrono',service_version:1,input:{delay_ms:1500}})});
 if(admission.status!==202||!admission.body.protocol_id)throw new Error(`ASYNC sem aceite durante indisponibilidade: HTTP ${admission.status} ${JSON.stringify(admission.body)}`);
 const protocolID=admission.body.protocol_id;
 const accepted=sql(`SELECT count(*)::text||E'\\t'||(SELECT status FROM protocols WHERE tenant_id='acme' AND idempotency_key='${key}' ORDER BY accepted_at DESC LIMIT 1) FROM protocols WHERE tenant_id='acme' AND idempotency_key='${key}'`);
 ...
 do{await new Promise(resolve=>setTimeout(resolve,750));current=(await request(token,`/v1/protocols/${encodeURIComponent(protocolID)}`)).body;if(['SUCCEEDED','FAILED','EXPIRED'].includes(current.status))break}while(Date.now()<deadline);
 ...
 const after=await fetch(`${providerSim}/__qualification/effects`).then(response=>response.json());
