...
func (c *Catalog) CreateSession(ctx context.Context, tenant string, q UploadRequest) (UploadSession, error) {
	...
	e = pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		_, e := tx.ExecContext(ctx, `INSERT INTO file_refs(id,tenant_id,object_key,sha256,size_bytes,content_type,purpose,class,region,state,expires_at,retention_until) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'UPLOADING',$10,$11)`, id, tenant, key, q.SHA256, q.Size, q.ContentType, q.Purpose, q.Class, q.Region, expires, time.Now().UTC().Add(retention))
		return e
	})
	if e != nil {
		return out, e
	...
	if uploadID != "" {
		if e = pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
			_, e := tx.ExecContext(ctx, `UPDATE file_refs SET upload_id=$3 WHERE tenant_id=$1 AND id=$2`, tenant, id, uploadID)
			return e
		}); e != nil {
			return out, e
		}
...
func (c *Catalog) Complete(ctx context.Context, tenant, id string, parts []CompletedPart) (FileRef, error) {
	...
	var n int64
	e = pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		res, e := tx.ExecContext(ctx, `UPDATE file_refs SET state='READY',object_version=$3,updated_at=clock_timestamp() WHERE tenant_id=$1 AND id=$2 AND state IN('UPLOADING','VALIDATING') AND expires_at>clock_timestamp()`, tenant, id, m.Version)
		if e != nil {
			return e
		}
...
func (c *Catalog) Pin(ctx context.Context, tenant, id, obligation, reason string) error {
	...
	return pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		var state string
...
func (c *Catalog) StoreResult(ctx context.Context, tenant, obligation string, q UploadRequest, reader io.Reader) (FileRef, error) {
	...
	e = pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		_, e := tx.ExecContext(ctx, `UPDATE file_refs SET state='ORPHAN',object_version=$3,sha256=$4,size_bytes=$5,updated_at=clock_timestamp() WHERE tenant_id=$1 AND id=$2 AND state='VALIDATING'`, tenant, id, m.Version, m.SHA256, m.Size)
		return e
	})
	if e != nil {
		return FileRef{}, e
...
func (c *Catalog) Reconcile(ctx context.Context, tenant string) (int, error) {
	var ids []string
	e := pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		rows, e := tx.QueryContext(ctx, `SELECT id FROM file_refs WHERE tenant_id=$1 AND state='VALIDATING' AND obligation_id IS NOT NULL ORDER BY created_at LIMIT 100`, tenant)
		if e != nil {
			return e
		}
		defer rows.Close()
