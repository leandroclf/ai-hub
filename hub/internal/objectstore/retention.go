...
func (c *Catalog) DryRun(ctx context.Context, tenant, actor string) (PurgePlan, error) {
	...
	e := pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		rows, e := tx.QueryContext(ctx, `SELECT id FROM file_refs f WHERE tenant_id=$1 AND state IN ('READY','ORPHAN','PURGING') AND retention_until<clock_timestamp() AND NOT EXISTS(SELECT 1 FROM object_retention_pins p WHERE p.file_id=f.id) ORDER BY id LIMIT 100`, tenant)
		if e != nil {
			return e
		}
		for rows.Next() {
		...
		_, e = tx.ExecContext(ctx, `INSERT INTO object_gc_runs(id,tenant_id,actor,candidates,expires_at) VALUES($1,$2,$3,$4,$5)`, out.ID, tenant, actor, string(b), out.ExpiresAt)
		return e
	})
	return out, e
}
...
func (c *Catalog) IsTombstoned(ctx context.Context, tenant, id string) (bool, error) {
	var found bool
	e := pg.WithTenantTx(ctx, c.db, tenant, func(tx *sql.Tx) error {
		return tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM object_tombstones WHERE tenant_id=$1 AND file_id=$2)`, tenant, id).Scan(&found)
	})
	return found, e
}
