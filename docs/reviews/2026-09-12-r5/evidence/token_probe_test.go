...
func TestR4WarmTokenSurvivesVaultOutage(t *testing.T){
 ...
 sum:=sha256.Sum256([]byte(strings.Join([]string{cfg.Environment,cfg.TenantID,cfg.BindingID,"account",cfg.TokenURL,cfg.ClientID,cfg.ClientSecretRef,cfg.SecretVersion},"\x1f")))
 key:="hub:provider-token:"+hex.EncodeToString(sum[:])
 c:=&TokenCache{Resolver:r4DownVault{},tokens:map[string]cachedToken{key:{value:"synthetic-valid",expires:time.Now().Add(time.Minute),environment:cfg.Environment,tenant:cfg.TenantID,binding:cfg.BindingID,version:cfg.SecretVersion}}}
