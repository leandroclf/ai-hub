...
| horizonte absoluto de retry | primeira falha fixa `retry_started_at`/`retry_until`; takeover não renova TTL; intenção expira | working tree atual; `core/0035_retry_horizon.sql`; imagem Órbita reconstruída | `R2_CORE_TEST_DSN=postgres://... go test ./internal/orbita -run TestPostgresRetryHorizonStartsOnceAndExpires -count=1 -v` | PASS PostgreSQL real em 1,14s; readiness Órbita HTTP 200 |
| observação conflitante | callback e polling concorrentes conservam ambas as receipts, classificam o conflito e mantêm um único fato terminal | working tree atual; Cometa reconstruído localmente | `R2_CORE_TEST_DSN=postgres://... go test ./internal/cometa -run TestPostgresPollingCallbackConflictRetainsBoth -count=1 -v` | PASS PostgreSQL real: 1 outbox terminal, 3 receipts totais, 1 receipt `*_CONFLICT` |
| worker de retenção | lote de expurgo com plano durável, retomada de `PURGING` e escopo explícito de tenants | `hub/internal/objectstore/retention.go`, `hub/cmd/orbita/main.go` | `cd hub && go test ./internal/objectstore ./cmd/orbita`; `git diff --check` | PASS unitário/compilação; execução destrutiva permanece opt-in por `RETENTION_TENANTS` |
| provider após reinício | deduplicação e oráculo de efeitos persistem fora do processo | `hub/internal/providersim/providersim.go`, volume `ai_hub_r3qual_provider-sim-data` | POST sintético + `docker compose restart provider-sim` + GET `/__qualification/effects` | PASS real: mesmo `provider_request_id`, `effects=1`, `protocols=1` após reinício |
| retenção efetiva | pin impede seleção; UNPIN auditado permite lote; tombstone e remoção S3 impedem leitura posterior | `hub/internal/objectstore/retention.go`, `catalog_test.go` | `R2_CORE_TEST_DSN=... R2_S3_TEST_ENDPOINT=... go test ./internal/objectstore -run TestPostgresS3MultipartFileRef -count=1 -v` | PASS PostgreSQL+LocalStack em 1,79s |
| restore com digest | dump/restore lógico e comparação de conteúdo determinístico em control/core/finance, com nomes S3 válidos | `hub/deploy/r2/tests/restore-reconciliation.sh` | `R2_RESTORE_SUFFIX=autonomous_digest2 bash hub/deploy/r2/tests/restore-reconciliation.sh` | PASS: contagens e digests iguais; bucket isolado com 0 objetos; oráculo externo observado sem replay |
| falha entre efeito e resposta | provider-sim grava efeito/chave e encerra conexão antes da resposta; Cometa pode injetar o cenário pelo payload | `hub/internal/providersim/providersim.go`, `hub/internal/cometa/executor.go` | `go test ./internal/providersim ./internal/cometa` | PASS unitário; ensaio integrado `UNKNOWN` + recuperação ainda pendente |
| recuperação externa após queda | imagem atual do provider-sim perde resposta após efeito e repete a mesma chave sem novo efeito | `hub/internal/providersim/providersim.go`, imagem `ai-hub-r2-provider-sim:r2` | POST `qualification-drop-after-effect-v4-20260909` com `drop_after_effect`, repetição e GET `/__qualification/effects` | PASS real: `SUCCEEDED`, mesmo `prov-req-000001`, `effects=1`, `protocols=1` |
| custódia integrada pós-efeito | Executor Cometa grava UNKNOWN durável e replay não reexecuta o provider | `hub/internal/cometa/custody_test.go` + PostgreSQL real + provider-sim httptest | `R2_CORE_TEST_DSN=... go test ./internal/cometa -run TestPostgresExecutorDropAfterEffectIsUnknownAndNotReexecuted -count=1 -v` | PASS: `transport_error`, mesma evidência no replay e um efeito externo |
