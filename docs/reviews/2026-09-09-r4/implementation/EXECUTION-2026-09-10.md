...
| Redis opcional / fallback L1 | `R2_CORE_TEST_DSN=... go test -race -count=1 ./internal/providerauth -run TestOAuthL1RemainsUsableWhenResolverBecomesUnavailable` | PASS unitário; a indisponibilidade do resolver Redis não bloqueou a reutilização limitada do token L1 nem provocou nova emissão (`R2-OPE-06-S02`) |
| Todas as autoridades fora | `node hub/deploy/r2/tests/authoritative-outage-runtime-proof.mjs` | PASS integrado local; PostgreSQL parado retornou `503 admission_unavailable` sem protocolo, a mesma chave foi repetida após recuperação e resultou em um único protocolo `SUCCEEDED` (`R2-OPE-06-S03`); o procedimento recriou dependentes com CIDRs reais do Compose |
| Fluxo rastreável | `node hub/deploy/r2/tests/traceability-runtime-proof.mjs` | PASS integrado local; ASYNC real correlacionou trace durável na intenção, logs de Órbita/Cometa e timeline administrativa, confirmou histograma de latência e ausência de IDs de negócio em labels (`R2-OPE-07-S01`) |
| Alerta de outbox envelhecida | `R2_COMPOSE_PROJECT=ai_hub_r3qual bash hub/deploy/r2/tests/observability-outbox-runtime-proof.sh` | PASS integrado local; broker foi interrompido, obrigação sintética envelhecida permaneceu pendente, o gauge PostgreSQL-backed superou 30 s, `HubOutboxDelayed` ficou `firing` no Prometheus e o cleanup restaurou o LocalStack/removou a fixture (`R2-OPE-07-S02`) |
| Oráculo de regressão do eco | `go test -race -count=1 -v ./internal/cometa -run 'TestQualificationEchoRegression'` | PASS unitário; fixture HTTP retornou `SUCCEEDED` com o input ecoado e o oráculo rejeitou o conteúdo em SYNC e ASYNC (`R2-QUA-02-S02`) |
| Oráculo de regressão financeira | `R2_FINANCE_DSN=... go test -race -count=1 -v ./internal/libra -run TestQualificationCapturedBalanceBlocksThirdExternalAttempt` | PASS integrado local; duas capturas consumiram `2.00000000 BRL`, a terceira falhou antes da chamada externa e o disponível ficou zero (`R2-QUA-02-S03`) |
| Replay antigo após expurgo lógico | `R2_CORE_TEST_DSN=... go test -race -count=1 -v ./internal/orbita -run TestOperationFactPostgresCustody` | PASS integrado local; o evento repetido após terminal encontrou inbox `APPLIED`/tombstone lógico, não criou nova finalização/outbox e identidade divergente foi para quarentena (`R2-DAD-03-S03`) |
| Requalificação SYNC com broker fora | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual R2_BROKER_OUTAGE_PREFIX=r4-broker-outage-20260911qual node hub/deploy/r2/tests/broker-outage-runtime-proof.mjs` | PASS local; broker indisponível foi observado, Orbita/Cometa permaneceram saudáveis após recriação, SYNC/GET devolveram `SUCCEEDED` em 200, `finalized_outbox=1` e o broker foi restaurado; `R2-EXE-07-S01` promovido a `PASS_INTEGRADO` |
| Crash após aceite | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual R2_CRASH_ACCEPTANCE_PREFIX=r4-crash-acceptance-20260911qual3 node hub/deploy/r2/tests/crash-after-acceptance-runtime-proof.mjs` | PASS local; aceite ASYNC em `ACCEPTED` foi persistido, Órbita reiniciada imediatamente voltou a `Ready`, a consulta pública chegou a `SUCCEEDED` e PostgreSQL/provider-sim confirmaram 1 protocolo, 1 operação, 1 fato terminal e 1 efeito |
| Broker fora antes do aceite assíncrono | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual R2_ASYNC_BROKER_OUTAGE_PREFIX=r4-async-broker-outage-20260911qual3 node hub/deploy/r2/tests/async-broker-outage-runtime-proof.mjs` | PASS local; o broker foi interrompido antes da admissão, o Hub respondeu `202/ACCEPTED`, a intenção permaneceu durável e, após restaurar o LocalStack e recriar Orbita/Cometa, a consulta retornou `SUCCEEDED`; PostgreSQL/provider-sim confirmaram 1 operação, 1 `protocol.finalized` e 1 efeito |
| Produto composto com etapas independentes | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual R2_PRODUCT_IDEMPOTENCY_KEY=r4-product-parallel-20260911qual2 node hub/deploy/r2/tests/product-runtime-proof.mjs` | PASS local; A/B concluíram com `max_parallel=2`, duas operações/efeitos reais e diferença de início de 35 ms; o produto consolidou `SUCCEEDED` e replay não criou novo plano ou efeito (`R2-CAT-03-S01`) |
| Produto parcial com falha opcional | `R2_CORE_TEST_DSN=... R2_FINANCE_DSN=... go test -race -count=1 -v ./internal/orbita -run TestProductOptionalFailurePublishesEligibleFinanceFacts` | PASS integrado local; PostgreSQL atravessou Orbita → outbox → Libra, confirmou A obrigatório `SUCCEEDED`, B opcional `FAILED`, protocolo `PARTIALLY_SUCCEEDED`, um custo elegível, uma receita parcial, quatro lançamentos balanceados e reserva `CAPTURED` (`R2-CAT-03-S02`) |
| Resolução e bloqueio de oferta | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/offer-resolution-runtime-proof.mjs` | PASS local; Atlas selecionou `prov-poll-2` para `app-acme` com prioridade 2, versão/vínculo/hash, recusou `prov-sync-1` fora da oferta com `403/offer_not_eligible`, bloqueou consulta cross-tenant e manteve em `DRAFT` ofertas com incompatibilidade de SLA/modalidade ou vínculo divergente (`R2-CAT-02-S03`) |
| Commit incerto e replay | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMMIT_UNCERTAIN_PREFIX=r4-commit-uncertain-20260911qual3 node hub/deploy/r2/tests/commit-uncertain-runtime-proof.mjs` | PASS local; cliente fechou a conexão após enviar o request, o aceite `ACCEPTED` foi recuperado, o replay devolveu o mesmo UUID e a cardinalidade permaneceu em um protocolo sem operação duplicada; a injeção dentro do commit PostgreSQL permanece uma limitação explícita |
| Reexecução do broker gate em 11/09 | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/broker-outage-runtime-proof.mjs` | NÃO QUALIFICADO nesta rodada; LocalStack e workloads foram restaurados, mas quatro obrigações `pending_external` órfãs bloquearam `r4-sync` e o SYNC respondeu `504`; nenhum estado externo foi encerrado por suposição |
...
| Quota/capacidade | `R2_CORE_TEST_DSN=... go test -count=1 -v ./internal/cometa -run 'TestPostgresCapacity...'` | PASS unitário PostgreSQL; 60 contendores em 3 réplicas/2 células, limites A/B preservados, owner stale cercado, reserva positiva e rate limit por tenant; autoscaling cloud permanece fora |
| Política de escala | `bash hub/deploy/r2/tests/scaling-policy-proof.sh` | PASS estrutural; KEDA usa backlog de entregas pendentes, HPA/PDB/topology spread estão presentes e ppd declara envelope 3→6; backlog real sob CPU baixa e provisionamento cloud permanecem fora |
| Realocação de tenant | `R2_CORE_TEST_DSN=... go test -count=1 -v ./internal/orbita -run TestPublicProtocolLookupSurvivesTenantCellRelocation` | PASS integrado local; protocolos antigos em A e novos em B resolvidos por UUID após realocação, tenant alheio ocultado e fence interno preservado |
| Fronteira de broker | `go test -count=1 -v ./internal/queue -run TestResourceNameIsolatesEnvironmentAndCell` | PASS unitário/estrutural; `QUEUE_NAMESPACE` separa recursos lógicos em dev/hom e células A/B; entrega runtime multiambiente permanece pendente |
| Readiness de provisionamento | `R2_CONTROL_TEST_DSN=... go test -count=1 -v ./internal/atlas -run TestRequestCapacityKeepsPartialCellOutOfPlacementAndIsIdempotent` | PASS integrado local; célula sem headroom qualificado permanece em `PROVISIONING`, sem placement e sem duplicação de solicitação |
| Alertas e runbooks | `bash hub/deploy/r2/tests/observability-alert-proof.sh` + ensaio controlado com Alloy fora | PARCIAL integrado; regras, runbooks, dashboard e montagem Kind passaram; `HubTelemetryDropped` ficou `firing` após 1001 descartes adicionais e Alloy foi recuperado; outbox/DLQ/SLA live permanecem pendentes |
| Reinício de nó Kind | `docker restart ai-hub-r2-worker2` + `continuity-runtime-proof.sh` | PARCIAL integrado; nó voltou a `Ready` em 1.641 ms e workloads/endpoints foram revalidados; recriação completa em máquina limpa permanece pendente |
| Falha de custódia S3 | `R2_CORE_TEST_DSN=... go test -count=1 -v ./internal/objectstore -run TestStoreResultKeepsObligationReconciliableWhenS3IsUnavailable` | PASS integrado local; falha do storage preservou `VALIDATING`, não criou `READY` e não confirmou sucesso fictício |
...
| Portal Playwright | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/browser-smoke.mjs` | PASS; 23 verificações PASS e 1 OBSERVED: OIDC PKCE, senha+OTP, erro de JSON inválido sem falso sucesso, editor de contrato REST declarativo, filtro local de referências, criação/leitura durável, conflito `If-Match` com edição local preservada e diff da revisão do servidor, editor de produto com mapeamento, SLA, redelivery administrativo com recibo/auditoria durável, reconciliação segura sem correlação externa, destinos versionados, financeiro, preparação de ajuste com bloqueio de autoaprovação, navegação, logout, HTTP 401 pós-logout sem credencial, storage sem sessão persistida, viewport móvel sem overflow e `tenant_reader` sem formulário de escrita |
| Ciclo de cliente administrativo | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-client-lifecycle-smoke.mjs` | PASS; pendência com capacidade 999 mostrou bloqueio de provisionamento e cliente publicado foi suspenso com razão e histórico acessível enquanto existia protocolo `RUNNING` sintético; cleanup removeu apenas as fixtures do ensaio |
...
| Contrato legado e imutabilidade de perfil | `R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/catalog-legacy-contracts-runtime-proof.mjs` + `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-legacy-contract-smoke.mjs` + `admin-service-publication-smoke.mjs` | PASS integrado local; dois contratos legados com campos/mapeamentos distintos normalizaram para o mesmo contrato técnico, terminaram `SUCCEEDED`, e GET/webhook foram equivalentes por cliente; publicação versionada preservou hash/revisão |
...
| Políticas técnicas administrativas | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-technical-policy-smoke.mjs` | PASS parcial; TTL de 300s foi limitado a 28s por SLA 30s e SYNC incompatível com `async_poll` foi bloqueado; contrato legado GET/webhook continua aberto |
| Resultado materializado e escopo de protocolos | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-protocol-scope-smoke.mjs` | PASS; final persistido foi consultado sem provider-sim e `leitor-a` não conseguiu ampliar filtro de `acme` para `beta` |
| Frescor e exportação do painel SLA | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-sla-freshness-smoke.mjs` | PASS; 2 verificações Chromium: painel exibiu `generated_at`, `watermark_at` e atraso real, e `sla-report.csv` foi limitado aos filtros autorizados com auditoria `EXPORT` |
| Ajuste compensatório administrativo | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-finance-adjustment-smoke.mjs` | PASS; ajuste exigiu razão, bloqueou autoaprovação e foi aprovado por `aprovador-a` distinto, no mesmo tenant e com recibo persistido |
| Usabilidade e recuperação administrativa | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-usability-recovery-smoke.mjs` | PASS; teclado levou foco ao resumo de erro com JSON preservado e revisão obsoleta retornou HTTP 412 com entrada local/diff recuperáveis |
| Escopo de sessão administrativa | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-session-scope-smoke.mjs` | PASS; `leitor-a` recebeu 403 ao solicitar `beta` na autoridade e o portal manteve contexto `acme` sem input cross-tenant |
| Contrato legado administrativo | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/admin-legacy-contract-smoke.mjs` | PASS; perfil JSON com polling, oferta com rotas polling/callback e comparação do mesmo corpo final no GET do protocolo e no detalhe da entrega webhook |
| Multipart incompleto | `AWS_ACCESS_KEY_ID=test ... R2_CORE_TEST_DSN=<fixture> R2_S3_TEST_ENDPOINT=http://127.0.0.1:14566 go test -tags=e2e -count=1 ./internal/objectstore -run TestPostgresS3MultipartFileRef -v` | PASS; confirmação com parte ausente retornou erro de integridade e a FileRef permaneceu `UPLOADING`, sem promoção para `READY` |
| Upload HTTP e submit com FileRef | `PLAYWRIGHT_MODULE=.../playwright-core/index.js R2_COMPOSE_PROJECT=ai_hub_r3qual node hub/deploy/r2/tests/file-upload-submit-smoke.mjs` | PASS; upload direto foi confirmado como `READY` por hash e a admissão real criou pin para o protocolo aceito; 504 posterior preservou UUID/consulta e não anulou a custódia |
| Finalização tardia | `R2_CORE_TEST_DSN=<fixture> go test -count=1 ./internal/orbita -run '^TestFinalizeLateSuccessBecomesExpired$' -v` | PASS; tentativa de sucesso após o deadline materializou `EXPIRED/SLA_EXCEEDED` sem resultado do provedor |
| Commit cruza o deadline | `R2_CORE_TEST_DSN=<fixture> go test -count=1 ./internal/orbita -run '^TestFinalizeCommitCrossingDeadlineDoesNotEscapeAsSuccess$' -v` | PASS; lock PostgreSQL iniciado antes do prazo e liberado depois produziu `EXPIRED/REJECTED_LATE_SLA`, sem resultado tardio e com um único fato terminal |
| Despacho órfão | `R2_CORE_TEST_DSN=<fixture> go test -count=1 ./internal/orbita -run '^TestRecoverOrphanedIntentRequeuesSameDurableObligation$' -v` | PASS; scanner encontrou lease expirado, registrou diagnóstico/auditoria e devolveu o mesmo comando ao publisher sem criar obrigação nova |
...
| Contratos JSON estritos | `hub/evidence/r2/execution/r4-ctr-02-latest.log` + `catalog-schema-upgrade-runtime-latest.json` + `catalog-snapshot-upgrade-runtime-latest.json` + `catalog-migration-rollback-latest.log` | PASS local; dialeto `ai-hub-json-schema-subset/v1`, enum numérico matemático, integer exato e limites de schema; upgrade/rollback preservou contrato v1, snapshot, obrigação em voo e replay idempotente; homologação externa e requalificação herdada permanecem abertas |
...
| R2-08/R2-09 complementar | `hub/evidence/r2/execution/rls-runtime-latest.log` + `kind-continuity-latest.log` + `slo-load-latest.json` + `broker-outage-runtime-latest.json` + `environment-boundary-latest.log` + `optional-dependency-runtime-latest.log` + `provider-outage-runtime-latest.json` + `capacity-quota-latest.log` + `scaling-policy-latest.log` | PASS seletivo; RLS nas três bases, bootstrap Kind independente, UI/gateway, recuperação de workloads, perfil de referência SLO, reinício com broker fora, gate de promoção, dependência opcional sem cascata, provedor fora contido por capacidade, quota por tenant e política declarada de escala; manutenção sob ruído, backlog real, autoscaling/provisionamento cloud, isolamento cross-environment integrado, ambientes remotos e HA regional permanecem abertos |
...
de laboratório com provider-sim, PostgreSQL e portal administrativo.

### Dois contratos legados com normalização equivalente — 11/09/2026

O ensaio `catalog-legacy-contracts-runtime-latest.json` publicou dois perfis
técnicos versionados para o mesmo serviço: o primeiro aceitou os campos
`cpf_legado` e `espera_ms`, enquanto o segundo aceitou `documento` e `atraso`.
Cada perfil usou seu próprio binding, conta e oferta, evitando ambiguidade de
roteamento. Duas admissões ASYNC reais confirmaram no PostgreSQL que os
comandos enviados ao provedor foram normalizados para `cpf` e `delay_ms`, sem
vazamento dos nomes legados, e ambas terminaram `SUCCEEDED`.

Para cada protocolo, a prova criou uma entrega representativa e consultou os
endpoints administrativos de Órbita e Pulsar. O corpo final do GET e o corpo
do webhook foram iguais após normalização canônica de JSON. A evidência
complementar `admin-legacy-contract-smoke.json` mantém a validação Chromium do
portal para o perfil legado e as modalidades polling/callback.

Esse resultado promove R2-CAT-05-S01 para `PASS_INTEGRADO_LOCAL` e encerra a
qualificação dos três cenários de R2-CAT-05. O limite permanece local:
provider-sim, PostgreSQL e o endpoint de webhook são fixtures de laboratório e
não substituem homologação comercial.
