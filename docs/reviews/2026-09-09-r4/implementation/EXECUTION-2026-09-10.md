...
| R2-08/R2-09 complementar | `hub/evidence/r2/execution/rls-runtime-latest.log` + `kind-continuity-latest.log` + `slo-load-latest.json` + `broker-outage-runtime-latest.json` + `environment-boundary-latest.log` + `optional-dependency-runtime-latest.log` | PASS seletivo; RLS nas três bases, bootstrap Kind independente, UI/gateway, recuperação de workloads, perfil de referência SLO, reinício com broker fora, gate de promoção e dependência opcional sem cascata; manutenção sob ruído, isolamento cross-environment integrado, ambientes remotos e HA regional permanecem abertos |
