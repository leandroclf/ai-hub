...
| Portal Playwright | `node hub/deploy/r2/tests/browser-smoke.mjs` | PASS; OIDC PKCE, senha+OTP, editor de contrato REST declarativo, filtro local de referências, criação/leitura durável, editor de produto com mapeamento, SLA, destinos versionados, navegação, logout, HTTP 401 pós-logout sem credencial, storage sem sessão persistida e viewport móvel sem overflow |
