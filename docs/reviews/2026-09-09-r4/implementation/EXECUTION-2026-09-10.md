...
| R2-08/R2-09 complementar | `hub/evidence/r2/execution/rls-runtime-latest.log` + `kind-continuity-latest.log` + `slo-load-latest.json` + `broker-outage-runtime-latest.json` | PASS seletivo; RLS nas três bases, bootstrap Kind independente, UI/gateway, recuperação de workloads, perfil de referência SLO e reinício com broker fora; manutenção sob ruído, ambientes remotos e HA regional permanecem abertos |
