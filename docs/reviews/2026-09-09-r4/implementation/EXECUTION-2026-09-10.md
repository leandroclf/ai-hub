...
| Produto parcial com falha opcional | `R2_CORE_TEST_DSN=... go test -race -count=1 -v ./internal/orbita -run TestProductOptionalFailureFinalizesPartialWithDurableStepStates` | PARCIAL integrado; PostgreSQL/inbox/finalizador confirmaram A obrigatório `SUCCEEDED`, B opcional `FAILED` e protocolo `PARTIALLY_SUCCEEDED`; incidência financeira de marcos elegíveis ainda não está fechada (`R2-CAT-03-S02`) |
