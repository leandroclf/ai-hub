...
| R2-08/R2-09 complementar | `hub/evidence/r2/execution/rls-runtime-latest.log` + `kind-continuity-latest.log` + `slo-load-latest.json` + `broker-outage-runtime-latest.json` + `environment-boundary-latest.log` + `optional-dependency-runtime-latest.log` + `provider-outage-runtime-latest.json` + `capacity-quota-latest.log` | PASS seletivo; RLS nas três bases, bootstrap Kind independente, UI/gateway, recuperação de workloads, perfil de referência SLO, reinício com broker fora, gate de promoção, dependência opcional sem cascata, provedor fora contido por capacidade e quota por tenant; manutenção sob ruído, autoscaling/provisionamento cloud, isolamento cross-environment integrado, ambientes remotos e HA regional permanecem abertos |
