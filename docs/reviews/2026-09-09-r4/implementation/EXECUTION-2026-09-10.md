...
| Resolução de oferta publicada | `PLAYWRIGHT_MODULE=.../playwright-core/index.js node hub/deploy/r2/tests/offer-resolution-runtime-proof.mjs` | PASS local; Atlas selecionou `prov-poll-2` para `app-acme` com prioridade 2, versão/vínculo/hash, recusou `prov-sync-1` fora da oferta com `403/offer_not_eligible` e bloqueou consulta cross-tenant |
