...
| `hub/evidence/r2/execution/compose-bootstrap-r2-ope-01-latest.log` | R2-OPE-01-S01 — primeira instalação local | PASS integrado local; bootstrap oficial reconciliou identidade, reaplicou migrations idempotentes, construiu as imagens atuais, iniciou as 16 unidades do projeto oficial, confirmou probes de aplicação/UI/IdP e aplicou CIDRs reais de egress | Volumes locais foram preservados; não qualifica máquina limpa, dependências gerenciadas, AWS ou recuperação de desastre |
| `hub/evidence/r2/execution/qualification-echo-regression-latest.log` | R2-QUA-02-S02 — regressão do eco | PASS unitário; fixture HTTP independente devolveu `SUCCEEDED` com o input ecoado em SYNC e ASYNC, e o oráculo de conteúdo rejeitou a resposta |
| `hub/evidence/r2/execution/qualification-finance-regression-latest.log` | R2-QUA-02-S03 — regressão financeira | PASS integrado local; PostgreSQL confirmou duas capturas consumindo o limite, terceira tentativa bloqueada antes da chamada externa e saldo disponível zero |
...
| `hub/evidence/r2/execution/redis-l1-fallback-latest.log` | R2-OPE-06-S02 — Redis lenta | PASS unitário; o teste de OAuth reutilizou o token L1 sem nova emissão quando o resolver Redis ficou indisponível | Não equivale a Redis gerenciado ou provedor comercial |
| `hub/evidence/r2/execution/authoritative-outage-runtime-latest.json` | R2-OPE-06-S03 — todas as cópias autoritativas indisponíveis | PASS integrado local; PostgreSQL parado foi recusado com `503 admission_unavailable` sem protocolo, e a repetição da mesma chave após recuperação produziu um único `SUCCEEDED` | Não qualifica HA regional ou banco distribuído |
| `hub/evidence/r2/execution/traceability-runtime-latest.json` | R2-OPE-07-S01 — fluxo rastreável | PASS integrado local; trace durável foi encontrado na intenção, nos logs de Órbita/Cometa e na timeline administrativa; histograma de latência presente e labels de métricas sem IDs de negócio | Não substitui observabilidade gerenciada ou investigação multi-região |
| `hub/evidence/r2/execution/message-replay-tombstone-latest.log` | R2-DAD-03-S03 — mensagem antiga | PASS integrado local; replay após terminal encontrou inbox `APPLIED`/tombstone lógico, não criou novo fato/outbox e conflito de identidade foi quarentenado | Não substitui retenção regional ou arquivamento produtivo |
| `hub/evidence/r2/execution/async-broker-outage-runtime-latest.json` | R2-EXE-01-S02 — aceite assíncrono com broker fora | PASS integrado local; `202/ACCEPTED` permaneceu durável durante a indisponibilidade, e a restauração/recriação dos consumidores levou a uma única operação, finalização e efeito externo |
| `hub/evidence/r2/execution/environment-boundary-latest.log` | R2-OPE-02-S02/S03 e R2-DAD-05-S03 — fronteira ambiental e gate de promoção | PASS estrutural para namespaces e referências sem segredos materializados; gate de `prd` bloqueia sem perfil/aprovações, bloqueia perfil RPO_ZERO com mera confirmação regional e exige custódia/fencing explícitos; dev permanece permitido; cross-environment integrado ainda parcial |
...
| `hub/evidence/r2/execution/observability-alert-latest.log` + `observability-alert-runtime-latest.log` + `observability-outbox-runtime-latest.log` | R2-OPE-07-S02 — alertas e runbooks | PASS integrado local; regras/runbooks/dashboard passaram, Alloy fora elevou `hub_telemetry_dropped_total` e o alerta ficou `firing`, e obrigação de outbox envelhecida com broker fora elevou o gauge PostgreSQL-backed e disparou `HubOutboxDelayed`; fixtures foram recuperadas e removidas |
...
| `hub/evidence/r2/execution/atlas-outage-runtime-latest.json` | R2-CAT-06-S02 | PASS integrado local; Atlas foi parado após aquecimento HTTP real, uma admissão reutilizou e persistiu projeção válida e a projeção vencida foi recusada com `403/offer_not_eligible` |
| `hub/evidence/r2/execution/catalog-import-failure-postgres.log` | R2-CAT-07-S02 | PASS integrado local; falha de leitura do inventário anterior retornou 503 antes da persistência do novo lote, preservando o staging anterior e o catálogo executável |
| `hub/evidence/r2/execution/catalog-snapshot-upgrade-runtime-latest.json` | R2-CAT-06-S01 | PASS integrado local; protocolo HTTP aceito contra serviço v1 permaneceu com target/contratos/perfil v1 após publicação da v2, terminou `SUCCEEDED` e gerou receita `sale-r4:1` com dois lançamentos contábeis |
| `hub/evidence/r2/execution/snapshot-admission-postgres.log` | R2-CAT-06-S01 | Evidência complementar; PostgreSQL congelou a projeção v1 no protocolo/intenção e não reinterpretou o aceite após alteração local para v2 |
| `hub/evidence/r2/execution/admin-service-publication-smoke.json` | R2-ADM-04-S02; R2-CAT-08-S02 | PASS integrado local; serviço com qualificação vigente foi validado sem chamadas ao provedor, publicado em v1, tentativa de mutação rejeitada com HTTP 409, preservando hash/estado, e `leitor-a` recuperou a mesma revisão após logout/refresh |
| `hub/evidence/r2/execution/admin-legacy-contract-smoke.json` | R2-ADM-07-S01 | PASS integrado local; um perfil legado JSON expôs polling/callback e o corpo canônico GET/webhook foi equivalente |
| `hub/evidence/r2/execution/catalog-legacy-contracts-runtime-latest.json` | R2-CAT-05-S01 | PASS integrado local; dois perfis legados usaram nomes/mapeamentos distintos, duas admissões reais normalizaram para o contrato do provedor, terminaram `SUCCEEDED` e cada GET administrativo foi equivalente ao webhook correspondente |
| `hub/evidence/r2/execution/catalog-schema-upgrade-runtime-latest.json` | R2-CAT-05-S03 | PASS integrado local; protocolo HTTP aceito com serviço v1 preservou snapshot, resultado e representação v1 enquanto versão incompatível removendo campos de entrada/saída foi publicada durante o processamento |
| `hub/evidence/r2/execution/admin-service-publication-smoke.json` | R2-CAT-05-S03 | Evidência complementar; Chromium criou/publicou v2 com schema incompatível, preservou v1/hash e rejeitou mutação direta |
...
| `hub/evidence/r2/execution/admin-product-simulation-smoke.json` | R2-CAT-03-S03 | PASS integrado local; produto com 21 etapas foi recusado pela autoridade com `máximo de 20 passos` antes da publicação e sem alterar contadores do provider-sim |
...
| `hub/evidence/r2/execution/admin-legacy-contract-smoke.json` | R2-ADM-07-S01 | PASS integrado local; Chromium confirmou input/output JSON e polling no perfil, rotas polling/callback na oferta e igualdade do corpo final entre detalhe do protocolo e entrega webhook |
...
| `hub/evidence/r2/execution/admin-usability-recovery-smoke.json` | R2-ADM-12-S01/S02 | PASS integrado local; teclado levou foco ao resumo de erro preservando JSON inválido e conflito HTTP 412 manteve entrada local/diff sem falso sucesso |
| `hub/evidence/r2/execution/admin-session-scope-smoke.json` | R2-ADM-01-S02 | PASS integrado local; `leitor-a` recebeu HTTP 403 para `tenant_id=beta` e a UI permaneceu em `acme`, sem input global nem dados de outro tenant |
...
| `hub/evidence/r2/execution/r2-int-exe-qualification.log` + `hub/internal/cometa/polling_custody_test.go` | R2-INT/R2-EXE selecionados; R2-CAT-06-S03 | PASS integrado local; polling HTTP real usou binding v1 do snapshot, revogação durante trabalho pendente bloqueou novo acesso sem novo efeito no provedor e encerrou recibo/capacidade; teste unitário de OAuth mantém a prova de isolamento L1 |
...
| `hub/evidence/r2/execution/late-success-expiry-smoke.json` | R2-EXE-06-S01 — final tardio | PASS integrado local; finalização após o deadline virou `EXPIRED/SLA_EXCEEDED` e não publicou o resultado do provedor |
| `hub/evidence/r2/execution/invalid-provider-output-unit.json` | R2-EXE-04-S02 — resposta incompatível | Evidência unitária complementar; normalização rejeita resposta sem campo exigido |
| `hub/evidence/r2/execution/invalid-provider-response-integration-latest.log` | R2-EXE-04-S02 — resposta incompatível | PASS integrado local; provedor HTTP retornou JSON inválido, Cometa conservou UNKNOWN sem sucesso/recibo bruto e não repetiu o POST |
| `hub/evidence/r2/execution/commit-failure-recovery-latest.log` | R2-EXE-04-S03 — commit após resposta válida | PASS integrado local; falha de commit terminal preservou obrigação UNKNOWN, correlação e recibo bruto, sem outbox e sem novo submit |
| `hub/evidence/r2/execution/public-representation-state-smoke.json` | R2-EXE-08-S03 — estados pendente/expirado | PASS integrado local; HTTP público serviu `WAITING_PROVIDER` e `EXPIRED` com 200, sem sucesso fictício e sem regenerar bytes finais |
| `hub/evidence/r2/execution/webhook-retry-representation-smoke.json` | R2-EXE-08-S01 — reentrega da representação | PASS parcial integrado; duas tentativas webhook verificaram os mesmos bytes, identidade e HMAC; composição única com POST/GET permanece aberta |
| `hub/evidence/r2/execution/result-reconciliation-smoke.json` | R2-DAD-02-S03 — objeto sem commit do protocolo | PASS integrado local; reconciliação validou objeto presente, recusou obrigação alheia e ligou somente a obrigação original, sem reexecução de provedor |
| `hub/evidence/r2/execution/finalize-expiry-race-smoke.json` | R2-EXE-06-S03 — disputa terminal | PASS integrado; PostgreSQL serializou finalização/expiração em uma transição e um único `protocol.finalized` |
| `hub/evidence/r2/execution/late-costly-result-finance-latest.log` | R2-EXE-06-S04 — resultado tardio custoso | PASS integrado local; GET/representação de webhook preservaram EXPIRED/SLA_EXCEEDED, Libra registrou só custo elegível, sem receita de sucesso, com contestação auditada |
| `hub/evidence/r2/execution/final-representation-immutable-smoke.json` | R2-EXE-08-S02 — imutabilidade histórica | PASS parcial integrado; tentativa de substituição após terminal foi recusada com bytes e versão preservados; upgrade real continua aberto |
...
| `hub/evidence/r2/execution/multipart-incomplete-smoke.json` | R2-DAD-01-S03 — confirmação multipart incompleta | PASS integrado local; parte ausente retornou erro de integridade e a referência permaneceu `UPLOADING`, sem promoção para `READY` |
| `hub/evidence/r2/execution/file-upload-submit-smoke.json` + `r2-cat-dad-qualification.log` | R2-DAD-01-S01/S02 | S01 PASS integrado local: upload HTTP, confirmação READY por hash e admissão com pin durável; S02 PASS local: download e resolução de FileRef entre tenants são recusados |
...
| `hub/evidence/r2/execution/product-http-latest.log` | Admissão HTTP de produto composto no provider-sim | PASS; duas etapas independentes produziram dois efeitos, `max_parallel=2`, início sobreposto em 35 ms, GET finalizou `SUCCEEDED` e repetição idempotente não criou novo efeito |
| `hub/evidence/r2/execution/product-optional-partial-finance-postgres.log` | R2-CAT-03-S02 — passo opcional com falha e incidência financeira | PASS integrado local; PostgreSQL atravessou Orbita → outbox → Libra, confirmou etapa obrigatória `SUCCEEDED`, opcional `FAILED`, um custo elegível, uma receita parcial, quatro lançamentos balanceados e reserva `CAPTURED` |
...
| `hub/evidence/r2/execution/product-http-latest.log` | R2-CAT-03-S01 | PASS integrado; A/B independentes respeitam `max_parallel=2`, iniciam com diferença de 35 ms e consolidam dois efeitos reais no mesmo produto |
...
| `hub/evidence/r2/execution/kind-continuity-latest.log` | Kind independente, bootstrap, UI/gateway, dependências, perda controlada de Cometa/Pulsar | PASS; três nós, dependências cluster-owned, cinco workloads, Jobs de migração/OIDC, RTO observado de 4.486 ms/4.494 ms; volume efêmero de laboratório; recriação completa de nó/máquina limpa permanece fora |
...
| `hub/deploy/r2/tests/generate-openspec-results.py` + `RESULT-MATRIX-732-CENARIOS.csv` | Matriz derivada do inventário: 732 linhas, 231 com evidência existente e 501 explicitamente `NAO_QUALIFICADO_NESTA_RODADA`; nenhum cenário sem prova é promovido |
...
| 11/09/2026: kind independente, readiness, métricas/HPA e recuperação de pod | laboratório `ai-hub-r2` / namespace `ai-hub-local-kind` | PASS local; 3 nós, 5 workloads, dependências `cluster-owned`, 2 réplicas distribuídas e recuperação de Cometa/Pulsar em 4.486/4.494 ms; não representa HA regional |
...
fonte: 732 cenários, dos quais 231 possuem resultado/evidência já registrada e
501 permanecem explicitamente não qualificados. O artefato é de rastreabilidade
