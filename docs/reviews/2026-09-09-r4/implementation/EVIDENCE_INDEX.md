...
| `hub/evidence/r2/execution/r2-security.log` + `r2-seg04-egress.log` | doze cenários R2-SEG-01/02/03/04 nomeados; PostgreSQL real na prova de auditoria e HTTPS/TLS local na prova de saída |
| `hub/deploy/r2/tests/generate-openspec-results.py` + `RESULT-MATRIX-732-CENARIOS.csv` | Matriz derivada do inventário: 732 linhas, 67 com evidência existente e 665 explicitamente `NAO_QUALIFICADO_NESTA_RODADA`; nenhum cenário sem prova é promovido |
...
fonte: 732 cenários, dos quais 67 possuem resultado/evidência já registrada e
665 permanecem explicitamente não qualificados. O artefato é de rastreabilidade
