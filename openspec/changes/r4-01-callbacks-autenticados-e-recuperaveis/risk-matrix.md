...
|---|---|---|---|---|
| O caminho local agora vincula callback conhecido à conta durável por HMAC-SHA256 v1 e não envia capability em URL. A composição da rota pública/gateway, rotação de chaves, mTLS e autenticação de provedor comercial ainda não foi homologada; a prova local não autoriza promoção externa. | P0 | R4-CBK-01 | Integrações e Core | R4-CBK-01-S01/S02/S03 |
