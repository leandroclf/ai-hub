...
Status: **R4-CBK-01/02/03/04 implementados e qualificados no laboratório; endpoint externo TLS/mTLS, rotação e promoção comercial permanecem abertas**. A suíte atual confirma custódia, inbox, recuperação, correlação, validação comum, HMAC versionado por conta e encaminhamento pelo Kong local na fixture.
## 1. Revalidação
...
- [x] 2.1 Fixar contrato e reproduzir contraexemplos.
  ...
  - Evidence: `hub/evidence/r2/execution/callback-account-auth-runtime-latest.json` demonstra que o segredo não vai na URL, que a conta é parte da autenticação e que a rota `/callbacks/*` foi encaminhada pelo Kong local.
- [x] 2.2 Implementar fluxo, dados e integração.
  ...
  - Evidence: `hub/internal/callbackauth`, migration `0044_callback_account_auth.sql`, Cometa, provider-sim e `hub/deploy/r2/kong.yml` implementam HMAC-SHA256 v1 por conta/operação/corpo, sem segredo na URL, com compatibilidade legada explícita e rota local integrada.
- [x] 2.3 Qualificar e anexar evidência.
  ...
  - Evidence: `hub/evidence/r2/execution/callback-account-auth-runtime-latest.json` e `traceability-runtime-latest.json`; prova local PASS, com limite explícito para endpoint TLS/mTLS externo, rotação e homologação comercial.
